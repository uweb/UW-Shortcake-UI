...
class Script extends Shortcode {
	...
	public static function get_shortcode_ui_args() {
		return array(
			'label'          => esc_html__( 'Script', 'uw-shortcake' ),
			...
			'attrs'          => array(
				array(
					...
					'attr'         => 'src',
					'type'         => 'text',
					'description'  => esc_html__( 'Full URL to the script file. Host must be whitelisted.', 'uw-shortcake' ),
				),
			),
		);
	}
	...
	*/
	public static function get_whitelisted_script_domains() {
		return apply_filters( 'uw_shortcake_whitelisted_script_domains', array() );
	}
	...
	public static function callback( $attrs, $content = '' ) {
		...
		if ( ! in_array( $host, static::get_whitelisted_script_domains() ) ) {
			if ( current_user_can( 'edit_posts' ) ) {
				return '<div class="shortcake-bakery-error"><p>' . sprintf( esc_html__( 'Invalid hostname in URL: %s', 'uw-shortcake' ), esc_url( $attrs['src'] ) ) . '</p></div>';
			} else {
				return '';
			}
		}
