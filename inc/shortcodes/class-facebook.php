...
class Facebook extends Shortcode {
	...
	public static function get_shortcode_ui_args() {
		return array(
			'label'          => esc_html__( 'Facebook', 'uw-shortcake' ),
			...
			'attrs'          => array(
				array(
					...
					'attr'         => 'url',
					'type'         => 'text',
					'description'  => esc_html__( 'Full URL to the Facebook Post or Video', 'uw-shortcake' ),
				),
			),
		);
	}
	...
	public static function setup_actions() {
		add_action( 'init', 'UW_Shortcake\Shortcodes\Facebook::action_init_register_scripts' );
	...
	public static function callback( $attrs, $content = '' ) {
		...
		if ( ! $match ) {
			if ( current_user_can( 'edit_posts' ) ) {
				return '<div class="shortcake-bakery-error"><p>' . sprintf( esc_html__( 'Invalid Facebook URL: %s', 'uw-shortcake' ), esc_url( $attrs['url'] ) ) . '</p></div>';
			} else {
				return '';
			}
		}
		...
		if ( ! has_action( 'wp_footer', 'UW_Shortcake\Shortcodes\Facebook::action_wp_footer' ) ) {
			add_action( 'wp_footer', 'UW_Shortcake\Shortcodes\Facebook::action_wp_footer' );
		}
