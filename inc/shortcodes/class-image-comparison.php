...
class Image_Comparison extends Shortcode {
	...
	public static function get_shortcode_ui_args() {
		return array(
			'label'          => esc_html__( 'Image Comparison', 'uw-shortcake' ),
			'listItemImage'  => 'dashicons-format-gallery',
			'attrs'          => array(
				array(
					'label'  => esc_html__( 'Left Image', 'uw-shortcake' ),
					'attr'   => 'left',
					'type'   => 'attachment',
				...
				array(
					'label'  => esc_html__( 'Right Image', 'uw-shortcake' ),
					'attr'   => 'right',
					'type'   => 'attachment',
					'libraryType' => array( 'image' ),
					'addButton'   => esc_html__( 'Select Image', 'uw-shortcake' ),
				...
				array(
					'label'  => esc_html__( 'Slider Start Position', 'uw-shortcake' ),
					'attr'   => 'position',
					'type'   => 'select',
					'options' => array(
						'center' => esc_html__( 'Center', 'uw-shortcake' ),
						'mostlyleft' => esc_html__( 'Mostly Left', 'uw-shortcake' ),
						'mostlyright' => esc_html__( 'Mostly Right', 'uw-shortcake' ),
					),
				),
			),
		);
	...
	public static function setup_actions() {
		add_action( 'wp_enqueue_scripts', 'UW_Shortcake\Shortcodes\Image_Comparison::action_init_register_scripts' );
		add_action( 'shortcode_ui_after_do_shortcode', function( $shortcode ) {
			if ( false !== stripos( $shortcode, '[' . self::get_shortcode_tag() ) ) {
				echo '<link rel="stylesheet" href="' . esc_url( UW_SHORTCAKE_URL_ROOT . 'assets/css/juxtapose.css' ) . '">';
				echo '<script type="text/javascript" src="' . esc_url( UW_SHORTCAKE_URL_ROOT . 'assets/js/juxtapose.js' ) . '"></script>';
			}
		});
	}

	public static function action_init_register_scripts() {
		wp_register_script( 'juxtapose-js', UW_SHORTCAKE_URL_ROOT . 'assets/js/juxtapose.js', array( 'jquery' ) );
		wp_register_style( 'juxtapose-css', UW_SHORTCAKE_URL_ROOT . 'assets/css/juxtapose.css' );
	}

	public static function callback( $attrs, $content = '' ) {
		if ( empty( $attrs['left'] ) || empty( $attrs['right'] ) ) {
			if ( current_user_can( 'edit_posts' ) ) {
				return '<div class="shortcake-bakery-error"><p>' . esc_html__( 'Two images required for image comparison.', 'uw-shortcake' ) . '</p></div>';
			} else {
				return '';
			}
		}
