...
class Playbuzz extends Shortcode {
	...
	public static function get_shortcode_ui_args() {
		return array(
			'label' => 'Playbuzz',
			'listItemImage' => '<img width="56px" height="85px" src="' . esc_url( UW_SHORTCAKE_URL_ROOT . 'assets/images/svg/icon-playbuzz.svg' ) . '" />',
			'attrs' => array(
				array(
					'label'        => esc_html__( 'Playbuzz quiz URL', 'uw-shortcake' ),
					'attr'         => 'url',
					'type'         => 'text',
					'description'  => esc_html__( 'The full URL to the Playbuzz quiz (e.g. https://www.playbuzz.com/Fusion/5-mind-blowing-facts-about-cloning-from-jurassic-park-youll-never-believe-actually-exist-in-real)', 'uw-shortcake' ),
				),
				array(
					'label'        => esc_html__( 'Display recommendations for more games', 'uw-shortcake' ),
					'attr'         => 'recommend',
					'type'         => 'checkbox',
					),
				array(
					'label'        => esc_html__( 'Display share buttons', 'uw-shortcake' ),
					'attr'         => 'shares',
					'type'         => 'checkbox',
					),
				array(
					'label'        => esc_html__( 'Use Facebook comments', 'uw-shortcake' ),
					'attr'         => 'comments',
					'type'         => 'checkbox',
					),
			),
	...
	public static function callback( $attrs, $content = '' ) {
		if ( empty( $attrs['url'] ) || ! preg_match( '#^https?://(www\.)?playbuzz\.com#', $attrs['url'] ) ) {
			if ( current_user_can( 'edit_posts' ) ) {
				return esc_html__( 'Invalid Playbuzz URL provided.', 'uw-shortcake' );
			} else {
				return '';
			}
		}
