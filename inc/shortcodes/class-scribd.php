...
class Scribd extends Shortcode {
	...
	public static function get_shortcode_ui_args() {
		return array(
			'label'          => esc_html__( 'Scribd', 'uw-shortcake' ),
			'listItemImage'  => '<img width="60px" height="auto" src="' . esc_url( UW_SHORTCAKE_URL_ROOT . 'assets/images/svg/icon-scribd.svg' ) . '" />',
			'attrs'          => array(
				array(
					'label'        => esc_html__( 'URL', 'uw-shortcake' ),
					'attr'         => 'url',
					'type'         => 'text',
					'description'  => esc_html__( 'Full URL to the Scribd document', 'uw-shortcake' ),
				),
			),
		);
	}
	...
	public static function callback( $attrs, $content = '' ) {
		...
		if ( ! preg_match( '#(http|https)://www\.scribd\.com/doc/?(\d)+/#', $attrs['url'], $needle ) ) {
			if ( current_user_can( 'edit_posts' ) ) {
				return '<div class="shortcake-bakery-error"><p>' . sprintf( esc_html__( 'Invalid Scribd URL: %s', 'uw-shortcake' ), esc_url( $attrs['url'] ) ) . '</p></div>';
			} else {
				return '';
			}
		}
